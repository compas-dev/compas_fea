...
#         state      = getattr(step, 'state', None)
        ...
        increments = getattr(step, 'increments')
...
#                 has_concrete = True
        ...
        if stype in ['GeneralStep', 'BucklingStep', 'ModalStep']:
            ...
            for k in step.loads:
                ...
                load  = loads[k]
                ltype = load.__name__
                com   = getattr(load, 'components')
                axes  = getattr(load, 'axes')
                nodes = getattr(load, 'nodes')
...
def _write_sofistik_loads_displacements(f, sets, displacements, loads, steps, structure):
    ...
    for k in sorted(loads):
        ...
        com   = getattr(load, 'components')
        axes  = getattr(load, 'axes')
        temp  = getattr(load, 'temperature')
        nodes = getattr(load, 'nodes')
        elset = getattr(load, 'elements')
